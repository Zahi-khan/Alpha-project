...
def _extract_narration_statement(pdf):
    """Read compact bank tables headed by Narration and split debit/credit columns.

    Some statements draw one visual row per transaction but expose the entire
    table body as newline-delimited cells. Reconstructing rows from word
    positions avoids depending on those collapsed cells.
    """
    records = []
    for page in pdf.pages:
        page_text = page.extract_text() or ""
        if not all(marker in page_text.casefold() for marker in ("narration", "withdrawal", "deposit")):
            continue
        words = page.extract_words(x_tolerance=2, y_tolerance=2)
        dates = sorted(
            (word for word in words if word["x0"] < page.width * .12 and _DATE.fullmatch(word["text"])),
            key=lambda word: word["top"],
        )
        for index, date_word in enumerate(dates):
            top = date_word["top"]
            next_top = dates[index + 1]["top"] if index + 1 < len(dates) else top + 34
            row_words = [word for word in words if top - 2 <= word["top"] < next_top - 2]
            withdrawal = _amount_in_relative_column(row_words, page.width, .63, .79)
            deposit = _amount_in_relative_column(row_words, page.width, .79, .90)
            balance = _amount_in_relative_column(row_words, page.width, .90, 1.01)
            if withdrawal is None and deposit is None:
                continue
            narration = _text_in_relative_column(row_words, page.width, .105, .45)
            if not narration:
                continue
            ...
            records.append({
                ...
                "Amount": amount,
                "Balance": balance or "",
                "Mode": _payment_mode(narration),
                "Type": "Credit" if deposit is not None else "Debit",
            })
    return dataframe_to_transactions(pd.DataFrame(records)) if records else []
...
def _amount_in_relative_column(words, width: float, start: float, end: float) -> str | None:
    return _amount_in_column(words, width * start, width * end)


def _text_in_relative_column(words, width: float, start: float, end: float) -> str:
    selected = [word for word in words if width * start <= word["x0"] < width * end]
